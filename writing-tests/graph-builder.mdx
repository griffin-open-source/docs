---
title: "Graph Builder"
description: "Build complex test flows with explicit control over execution order"
---
...
For simple linear tests, see the [Sequential Builder](/writing-tests/sequential-builder).
...
```typescript
import { HttpRequest, GET, POST, Json } from "griffin-ts";
...
```typescript
import { Assertion, Assert } from "griffin-ts";

// Create a state proxy to reference previous node results
import { createStateProxy } from "griffin-ts";
...
```typescript
import { START, END } from "griffin-ts";
