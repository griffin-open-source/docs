...
The graph builder gives you full control over execution flow. You define nodes (HTTP requests, browser steps, waits, assertions) and edges (the order they run in). This is the right choice when you need branching, parallel paths, or non-linear flows.

For simple linear monitors, see the [Sequential Builder](/writing-tests/sequential-builder). For browser-based monitoring, see [Browser Monitors](/writing-tests/browser-monitors).
...
| `base` | `string`, `variable()`, or `template` | Yes | Base URL (e.g., `"https://api.example.com"`) |
| `path` | `string`, `variable()`, or `template` | Yes | URL path (e.g., `"/users"`) |
| `response_format` | `Json`, `Xml`, `Text`, `NoContent` | Yes | Expected response format (`NoContent` for 204 with no body) |
...
See [Browser Monitors](/writing-tests/browser-monitors) for the full step reference and BrowserAction config.
