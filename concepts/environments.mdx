...
Each monitor is deployed to a specific environment. The same test file can be deployed to multiple environments with different configuration.
...
In your test files, reference variables with the `variable()` function:

```typescript
import { createTestBuilder, GET, Json, variable, Frequency } from "griffin-ts";
