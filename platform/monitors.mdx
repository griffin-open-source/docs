---
title: "Monitors"
description: "Managing your deployed API tests"
---

A monitor is a deployed test that runs on a schedule. When you run `griffin apply`, your local test files are pushed to the hub as monitors.

## Monitor lifecycle

```mermaid
flowchart LR
    A["Write test"] --> B["griffin plan"] --> C["griffin apply"] --> D["Scheduled execution"] --> E["Results"]
```

1. **Write** test files in `__griffin__/` directories
...
Edit your test files and re-deploy:

```bash
# Make changes to your test files, then:
...
<Warning>
`--prune` deletes any monitors on the hub that don't have a corresponding local test file. Make sure your local test files are complete before using this flag.
</Warning>
...
- **Name** — the `name` field in your test builder

This means the same test file can be deployed to multiple environments independently.
